...
public class Stocks {
    boolean flag=false;
    public boolean checkStock(String codeproduit, Statement stmt){
        try{
            String query="SELECT * FROM currentstocks where codeproduit='"+codeproduit+"'";
    ...
    public void deleteStock(Statement stmt){
         try{
             String q="DELETE FROM currentstocks WHERE codeproduit NOT IN(SELECT codeproduit FROM purchaseinfo)";
             String q1="DELETE FROM salesreport WHERE codeproduit NOT IN(SELECT codeproduit FROM products)";
